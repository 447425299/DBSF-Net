...
from .detail_extraction import DetailFeatureExtractor
from .base_extraction import BaseFeatureExtraction
...
def define_F(input_nc, netF, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    ...
    elif netF == 'base_sample':
        net = BaseSampleF(use_mlp=True, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids, nc=opt.netF_nc)
    elif netF == 'detail_sample':
        net = DetailSampleF(use_mlp=True, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids, nc=opt.netF_nc)
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    ...
    # elif netD == "combine":
    #     net = LocalGlobalDiscriminator(input_nc, ndf, n_layers=3, norm_layer=norm_layer, no_antialias=no_antialias)
...
# 这段代码是一个名为 PatchSampleF 的 PyTorch 模块，用于执行特征的采样。
# 主要的功能是从输入特征中采样一些特定的区域或路径，并将这些采样的特征进行处理，最后输出处理后的特征、采样的区域 ID，以及相应的注意力矩阵。
...
class BaseSampleF(nn.Module):
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[]):
        # potential issues: currently, we use the same patch_ids for multiple images in the batch
        super(BaseSampleF, self).__init__()
        self.l2norm = Normalize(2)
        self.use_mlp = use_mlp
        self.nc = nc  # hard-coded
        self.mlp_init = False
    ...
    def forward(self, feats, num_patches=64, patch_ids=None, attn_mats=None):
        ...
        if self.use_mlp and not self.mlp_init:
            self.create_mlp(feats)
        global_extractor = BaseFeatureExtraction(num_heads=2, ffn_expansion_factor=2, qkv_bias=False)
        for feat_id, feat in enumerate(feats):
            feat = global_extractor(feat.to(feat.device))
            B, C, H, W = feat.shape[0], feat.shape[1], feat.shape[2], feat.shape[3]
            feat_reshape = feat.permute(0, 2, 3, 1).flatten(1, 2) # B*HW*C
            if num_patches > 0:
                if feat_id < 3:
                ...
                patch_id = []
            if self.use_mlp:
                mlp = getattr(self, 'mlp_%d' % feat_id).to(feat.device)
                x_sample = mlp(x_sample)
            return_ids.append(patch_id)
            return_mats.append(attn_qs)
            x_sample = self.l2norm(x_sample)
...
class DetailSampleF(nn.Module):
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[]):
        # potential issues: currently, we use the same patch_ids for multiple images in the batch
        super(DetailSampleF, self).__init__()
        self.l2norm = Normalize(2)
        self.use_mlp = use_mlp
        self.nc = nc  # hard-coded
        self.mlp_init = False
    ...
    def forward(self, feats, num_patches=64, patch_ids=None, attn_mats=None):
        ...
        if self.use_mlp and not self.mlp_init:
            self.create_mlp(feats)
        detail_extractor = DetailFeatureExtractor(num_blocks=3, hidden_channels=64, kernel_size=3)
        for feat_id, feat in enumerate(feats):
            feat = detail_extractor(feat.to(feat.device))
            B, C, H, W = feat.shape[0], feat.shape[1], feat.shape[2], feat.shape[3]
            feat_reshape = feat.permute(0, 2, 3, 1).flatten(1, 2) # B*HW*C
            if num_patches > 0:
                if feat_id < 3:
...
#         # print("wavelet_coeffs.size:", wavelet_coeffs.size())
...
#         # print("wavelet_sum.size:", wavelet_sum.size())
#         wavelet_sum = torch.squeeze(wavelet_sum, dim=1)
#         # print("wavelet_sum.size:", wavelet_sum.size())
...
#         coeffs = pywt.dwt2(input.detach().cpu().numpy(), self.wavelet)
#         LL, (LH, HL, HH) = coeffs
#
...
#
#         self.net1 = NLayerDiscriminator(input_nc, ndf, n_layers, norm_layer, no_antialias)
#         self.net2 = WaveletDiscriminator(input_nc, ndf, wavelet='haar', n_layers=3, norm_layer=nn.BatchNorm2d)
#
#     def forward(self, input):
#         out1 = self.net1(input)
#         out2 = self.net2(input)
#         out2_resized = F.interpolate(out2, size=out1.size()[2:], mode='nearest')
#         # print("out1 size:", out1.size())
#         # print("out2 size:", out2.size())
#
#         return 0.5 * out1 + 0.5 * out2_resized
